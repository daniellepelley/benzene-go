// Command azure-functions-mesh-inventory is the inventory Azure Function - a pure event
// consumer. It reads order:placed off its own Event Hub consumer group (see
// examples/azure-functions-mesh/README.md's topology diagram) and shipment:dispatched off an
// Event Grid subscription. Handlers are trivial acknowledgements (domain.AckHandler) - the point
// of this example is proving the mesh's transport wiring, not rich domain behaviour.
package main
...
func newApp(meshClient *httpclient.Client) *meshapp.App {
	return meshapp.New(meshapp.Config{
		ServiceName: domain.ServiceInventory,
		...
		Register: func(registry *benzene.Registry, outbound *mesh.OutboundRegistry) []httpbinding.Route {
			...
			if err := domain.RegisterOutbound(outbound, domain.ServiceInventory); err != nil {
				log.Fatalf("register outbound for %s: %v", domain.ServiceInventory, err)
			}
			return nil
		},
	})
...
func mux(app *meshapp.App) http.Handler {
	...
	m.Handle("/ShipmentDispatched", azurefunctions.EventGridHandler(app.Builder(), "eventGridEvent"))
	return m
}
