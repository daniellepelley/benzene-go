...
// forwardHandler builds a handler that publishes req to sqsClient under topic "greet" and
...
// invocations connected only by the queue.
func forwardHandler(sqsClient *awssqs.Client) benzene.Handler[greeting.GreetRequest, struct{}] {
	return func(ctx context.Context, req greeting.GreetRequest) benzene.Result[struct{}] {
		...
		result := sqsClient.Send(ctx, benzene.NewTopic("greet"), nil, body)
...
func newApp(sqsClient *awssqs.Client) *benzene.ApplicationBuilder {
	registry := benzene.NewRegistry()
	if err := benzene.Register(registry, benzene.NewTopic("greet"), forwardHandler(sqsClient)); err != nil {
