// Command aws-lambda-mesh-inventory is the inventory Lambda: a pure event consumer subscribed to
// order:placed (SNS) and shipment:dispatched (EventBridge) - see
// examples/aws-lambda-mesh/README.md's topology diagram. It has no outbound sends at all; its
// only job is to acknowledge (domain.AckHandler) and report itself to the mesh.
package main
