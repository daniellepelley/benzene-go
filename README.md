...
| `clienthealthcheck` | 100% | Consumer-side dependency health check (zero deps), matching `Benzene.Clients.HealthChecks`: a `ServiceCheck` probes a downstream Benzene provider via an outbound `client.Sender` and reports the contract relationship - unreachable → failed, reachable+matching hash → ok, reachable+drifted hash → warning (degraded, doesn't flip health). `WithExpectedContractHash` compares the provider's live `descriptorHash` (from its `benzene:mesh` descriptor) against the hash the consumer was built against. For a contracts diagnostic surface, not a liveness probe |
