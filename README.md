...
| `resilience` | 100% | Retry middleware (zero deps), matching `Benzene.Resilience` (retry-only; circuit breaker/timeout/hedging/fallback are Polly's job in .NET, deferred here): `Middleware(opts...)` re-invokes the downstream pipeline with exponential backoff. Two retry triggers, since the router funnels failures onto `ic.Result` not a Go error - `WithRetryOnError` (default: any non-cancellation error) and `WithRetryOnResult` (default: never; pass `RetryUnsuccessful` or `RetryOnStatus(...)`). Backoff caps/jitters the sleep while growing the curve uncapped (AWS "full jitter", `FullJitter` helper); context-cancellable sleep. Place above idempotent outbound calls |
...
| `cloudservice` | 100% | One-call Cloud Service Profile **builder** (zero deps), the assembly counterpart of `cloudserviceprobe` and the Go form of `Benzene.CloudService`: `New(name, registry, opts...)` wires the reserved `/benzene/*` surface with conformant defaults - envelope-invoke (R2), health + `/benzene/health` (R3), the `benzene:mesh` descriptor (R6-inbound) and `/benzene/spec` (R5), plus the app routes - over one `ApplicationBuilder`, and returns the `http.Handler`, `Descriptor`, `Builder`, and a wiring `ProfileReport` (`Satisfied()`/`Unsatisfied()`; surfaces are Required/Recommended/Informational). `WithoutDescriptor()` drops R5/R6 per §4 exposure control. A thin assembler over the existing pieces; the outbound register/heartbeat feeds keep their explicit push-exporter lifecycle (reported Informational) |
