...
| `awseventbridge` ([own module](RELEASING.md)) | 97%+ | AWS EventBridge binding: inbound `Handler` for a Lambda invoked by a rule (zero deps; `detail-type` ↔ topic so rules pattern-match per topic, envelope-shaped `detail` unwrapped so wire headers travel; a failed event returns a Go error, triggering AWS's async-invoke retry) and an outbound `Client` publishing via `PutEvents` (needs `aws-sdk-go-v2/service/eventbridge`) |
...
`awssns`, `awseventbridge`, `kafka`, `diagnostics`, `examples/aws-sqs-helloworld`, and
`examples/aws-sns-helloworld`, which have their own `go.mod` because they need real third-party
dependencies the rest of the repo shouldn't carry. `go.work` ties them together for local
development.
