...
vs. stays consistent across language ports, see [work/archive/go-idioms-review.md](work/archive/go-idioms-review.md).)
...
| `validation` | 100% | Request-validation building block (zero deps): `Validated(validator, handler)` wraps a handler so an invalid request short-circuits to a `validation-error` result before the handler runs (`Validator[T]`/`ValidatorFunc[T]` reporting `[]benzene.Error` so a rule names its `Field` and `Code`, `Messages(...)` to adapt a plain `[]string` validator, and a `Combine` composer). The Go-idiomatic form of `Benzene.DataAnnotations`/`Benzene.FluentValidation`'s ValidationMiddleware - a typed handler wrapper, since this port's pipeline is type-erased until dispatch |
...
| `gcppubsub` | 100% | Google Cloud Pub/Sub inbound binding (zero deps): an `http.Handler` for a push subscription's endpoint - decodes the push envelope, resolves the topic per wire-contracts §2 (`topic` attribute or envelope-in-body), acks with 204 / nacks with 500 so Pub/Sub's own redelivery/dead-letter machinery handles failures. Outbound publishing is `gcppubsubclient`, its own module (it needs the Pub/Sub SDK) |
...
| `diagnostics` ([own module](RELEASING.md)) | 100% | OpenTelemetry diagnostics middleware - one server span per invocation (named by topic, joined to the caller's W3C `traceparent`, `benzene.topic`/`benzene.version`/`benzene.status` attributes) plus `benzene.messages.processed`/`benzene.message.duration` metrics (topic/transport/result-attributed, the cross-port [observability conventions](https://github.com/daniellepelley/Benzene/blob/main/docs/guides/observability-conventions.md)), and `WithTraceContext` (the OTel-path outbound client decorator that injects the active span context as a `traceparent`, sibling of `mesh.WithTraceContext`). Depends on the OTel *API* only; your app owns the SDK/exporter, and with no SDK installed the no-op defaults make it free |
...
designed in [mesh.md](work/archive/mesh.md) (archived now that it has shipped, along with a
[static mockup](work/archive/mesh-view-mockup.html) of the Fleet Overview screen and the
[research and positioning](work/archive/mesh-research.md) behind it). All phases of its
