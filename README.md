...
| `healthcheck` | 100% | Middleware that intercepts the reserved `healthcheck` topic and responds with the standard aggregate health response, plus ready-made `Check`s for dependency probes: `TCPCheck` (opens a connection, `Benzene.HealthChecks.Tcp`) and `HTTPPingCheck` (GET, healthy only on 200, URL credentials stripped, `Benzene.HealthChecks.Http`) - both zero-dep and report a coarse error category, never the raw message (`Benzene.HealthChecks.Disk` deferred: no portable free-space API in Go) |
...
| `resilience` | 100% | Retry + timeout middleware (zero deps), matching `Benzene.Resilience` (circuit breaker/bulkhead/hedging/fallback are Polly's job in .NET, deferred here as they want a library): `Middleware(opts...)` re-invokes the downstream pipeline with exponential backoff. Two retry triggers, since the router funnels failures onto `ic.Result` not a Go error - `WithRetryOnError` (default: any non-cancellation error) and `WithRetryOnResult` (default: never; pass `RetryUnsuccessful` or `RetryOnStatus(...)`). Backoff caps/jitters the sleep while growing the curve uncapped (AWS "full jitter", `FullJitter` helper); context-cancellable sleep. `Timeout(d)` bounds the downstream to a deadline (a cooperative `context.WithTimeout`, presented as a `StatusTimeout` result) - the one piece of the richer toolkit needing no library. Place above idempotent outbound calls |
| `circuitbreaker` ([own module](RELEASING.md)) | 100% | Circuit-breaker middleware, the library-backed slice of `Benzene.Resilience.Polly` (needs `sony/gobreaker/v2`, hence its own module): `Middleware[T](cb, opts...)` runs the downstream inside a gobreaker `CircuitBreaker` - a `next()` error or an unsuccessful `ic.Result` (per `WithTripOnResult`, default `TripUnsuccessful`; `TripOnStatus(...)`) counts as a failure; once open it **short-circuits without invoking the downstream** to a fail-fast status (`WithOpenStatus`, default `service-unavailable`). Open-state detected via a `called` flag (robust vs a downstream returning gobreaker's own sentinels); fail-fast result built at wiring time. Complements the zero-dep `resilience` (retry + timeout); bulkhead/hedging/fallback still deferred |
| `auth` | 100% | Authentication/authorization building block (zero deps), matching `Benzene.Auth.Core`+`.Basic`: a `Principal` (name/roles/claims) threaded on the context, `BasicAuth(validate, realm)` RFC 7617 authentication middleware (validates via an app-supplied `BasicValidator`, short-circuits `unauthorized` with a `WWW-Authenticate` challenge, or sets the principal + calls next), and `Authorize(predicate)`/`RequireRole(role)` authorization middleware (`forbidden` when not permitted, `unauthorized` when absent) |
