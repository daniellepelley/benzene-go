...
| `diagnostics` ([own module](RELEASING.md)) | 100% | OpenTelemetry diagnostics middleware - one server span per invocation (named by topic, joined to the caller's W3C `traceparent`, `benzene.topic`/`benzene.version`/`benzene.status` attributes) plus `benzene.messages.processed`/`benzene.message.duration` metrics (topic/transport/result-attributed, the cross-port [observability conventions](https://github.com/daniellepelley/Benzene/blob/main/docs/guides/observability-conventions.md)), and `TraceContextDecorator` (the OTel-path outbound client decorator that injects the active span context as a `traceparent`, sibling of `mesh.TraceContextDecorator`). Depends on the OTel *API* only; your app owns the SDK/exporter, and with no SDK installed the no-op defaults make it free |
